        ...
        if not csv_text.strip():
            ...
            try:
                ...
                if missing_feats:
                    ...
                    # Highlight rows based on fraud prediction
                    def highlight_row(row):
                        # Red for fraud, Green for legitimate (with opacity for readability)
                        color = 'rgba(255, 75, 75, 0.3)' if row['is_fraud'] else 'rgba(46, 125, 50, 0.3)'
                        return [f'background-color: {color}'] * len(row)
                    
                    st.dataframe(
                        df_batch.style.apply(highlight_row, axis=1),
                        use_container_width=True
                    )
